...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/mostafacbrx/3x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls https://raw.githubusercontent.com/mostafacbrx/3x-ui/master/install.sh") v$tag_version"
